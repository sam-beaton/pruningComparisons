function quality_matrices = prepQtnirs(nirsFilePath,varargin)
...
% end
%
% Adaptation of qtnirs function by SLB 20/10/24
% Edit 1:   Create assessTimeWindows (local function)
% Edit 2:   Create detectMotionWindows (local function)
% Edit 3:   Calculate good_combo_link (and, ultimately, MeasListAct) in
%           qualityCompute based only on windows without motion
% Edit 4:   Calculate, and output, mean SCI and PSP values across all
%           *valid (non-motion)* windows and add the values to qualityMats

if exist('dotNirsFilePath','var')==0
    nirsFilePath = pwd;
end
if ischar(nirsFilePath)
    if isfile(nirsFilePath)
        [filepath,name,ext] = fileparts(nirsFilePath);
        switch ext
            case '.nirs'
                rawNirs = load(nirsFilePath,'-mat');
            case '.snirf'   
                rawSnirf = SnirfClass(nirsFilePath);
...
%         end
    elseif isfolder(nirsFilePath)
        disp(['The input data is a folder. ',...
            'All .nirs files inside ',nirsFilePath,' will be evaluated.']);
        qtnirsLoadFileGUI(nirsFilePath);
        return;
    else
        error('The input path does not exist.');
    end
elseif isstruct(nirsFilePath)
    rawNirs = nirsFilePath;
...
elseif isa(nirsFilePath,'nirs.core.Data')
    rawNirs = struct;
    rawNirs.SD = nirs.util.probe2sd( nirsFilePath.probe );
    rawNirs.d = nirsFilePath.data;
    rawNirs.t = nirsFilePath.time;
    rawNirs.s = false(size(rawNirs.t));
    for icond = 1:length(nirsFilePath.stimulus.keys)
        stim = nirsFilePath.stimulus( nirsFilePath.stimulus.keys{icond} );
...
%            raw.d = raw.procResult.dod;
        ...
        %find time windows which contain motion
        [qualityMats.excludeMatrix, channelWindowMarked] = assessTimeWindows(raw, window, overlap, fs);

        qualityMats.channelWindowMarked = channelWindowMarked';
        ...
        % Initialize the 2 x n_windows vector for storing the first and last timepoints
        window_times = zeros(2, n_windows); 
        ...
        for j = 1:n_windows
            ...
            % Get the start and end timepoints for this window
            window_times(1, j) = raw.t(interval(1)); % first
...
%             end
        ...
        % Save window_times to qualityMats
        qualityMats.window_times = window_times;
        ...
        % Use excludeMatrix to filter acceptable windows for combo_array
        valid_windows = qualityMats.excludeMatrix == 1; % Get valid windows 
        
        % Update combo_array based on valid windows
        combo_array_valid = combo_array & valid_windows; 
        
        % Initialize arrays to hold averages
        avg_sci_channel = zeros(size(sci_array, 1), 1); % One average per channel
        avg_psp_channel = zeros(size(power_array, 1), 1); % One average per channel
        
        % Calculate averages for valid windows for each channel
        for channel_idx = 1:size(sci_array, 1)
            %get valid columns for the current channel
            valid_column_indices = find(valid_windows(channel_idx, :)); % Find indices of valid windows for this channel
        
            % Calculate the average for the valid windows
            avg_sci_channel(channel_idx) = mean(sci_array(channel_idx, valid_column_indices), 'omitnan'); % Average SCI per channel
            avg_psp_channel(channel_idx) = mean(power_array(channel_idx, valid_column_indices), 'omitnan'); % Average PSP per channel
        end

        % Add the average SCI and PSP values to qualityMats
        qualityMats.avg_sci_channel = avg_sci_channel;
        qualityMats.avg_psp_channel = avg_psp_channel;

        % adjusted stats for channels etc based on valild channels
        mean_combo_link  = mean(combo_array_valid,2); % Adjusted to consider only valid windows
        std_combo_link  = std(combo_array_valid,0,2);
        
        good_combo_link  = mean(combo_array_valid(:,idxPoi),2); % This will now only be based on valid windows
        mean_combo_window  = mean(combo_array_valid,1);
        std_combo_window  = std(combo_array_valid,0,1);
        
        idx_gcl = good_combo_link >= qltyThld;
        good_combo_window = mean(combo_array_valid(idx_gcl,:),1); % Ensure to use the valid combo array
        ...
        qualityMats.valid_windows   = valid_windows;
        qualityMats.combo_array     = combo_array;
        qualityMats.combo_array_valid = combo_array_valid;
...
function [excludeMatrix, channelWindowMarked] = assessTimeWindows(nirs, window_, overlap_, fs)
    ...
    % Convert window and overlap from seconds to samples using fs
    window_size = floor(window_ * fs); % Window size in samples
    overlap_size = floor(overlap_ * fs); % Overlap size in samples

    % Calculate the number of windows based on the overlap method used in qualityCompute
    if overlap_ == 0
        n_windows = floor(timepoints / window_size);
    else
        n_windows = 2 * floor(timepoints / window_size) - 1;
    end

    % Initialize exclusion matrix
    channelWindowMarked = zeros(n_windows, n_total_channels); % For each channel
    ...
    for iWin = 1:n_windows
        if overlap_ ~= 0
            if mod(iWin, 2) == 0
                jj = floor((iWin - 1) / 2) + 1;
                start_idx = (jj - 1) * window_size + overlap_size + 1;
            else
                jj = floor(iWin / 2) + 1;
                start_idx = (jj - 1) * window_size + 1;
            end
        else
            start_idx = (iWin - 1) * window_size + 1;
        end
        end_idx = min(start_idx + window_size - 1, timepoints);
        
        % Check if any timepoint in the window has tIncCh == 0 for each channel
        for iCh = 1:n_total_channels
            if any(tIncCh(start_idx:end_idx, iCh) == 0)
                channelWindowMarked(iWin, iCh) = 1; % Mark window for exclusion
            end
        end
    end

    % Generate the [n_windows x n_channels] matrix
    excludeMatrix = zeros(n_windows, n_channels);
    for iCh = 1:n_channels
        % Mark if either the channel or its pair (iCh + n_channels) was marked
        excludeMatrix(:, iCh) = channelWindowMarked(:, iCh) | channelWindowMarked(:, iCh + n_channels);
    end
...
%--------------------------------------------------------------------------
% Local function to detect motion windows
function motionWindows = detectMotionWindows(excludeMatrix, n_channels, consecutiveWindows)
    % Initialize the motion windows matrix
    [n_windows, ~] = size(excludeMatrix);
    motionWindows = zeros(size(excludeMatrix));
    
    % Criterion: Channel specific motion - check for 'consecutiveWindows' or more consecutive windows
    for channel = 1:n_channels
        for window = 1:(n_windows - consecutiveWindows + 1)  % Ensure we don't go out of bounds
            if all(excludeMatrix(window:window + consecutiveWindows - 1, channel) == 1)
                motionWindows(window:window + consecutiveWindows - 1, channel) = 1; % Mark the windows
            end
        end
    end
end
